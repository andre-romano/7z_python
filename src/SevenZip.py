...
class SevenZip:
    ...
    def run(self, arguments: list):
        ...
        command = [sevenZip_exe]+arguments
        ...
        # Create a queue and lock for multiprocessing
        queue = Queue()
        lock = Lock()

        # Start the worker process
        logger.info("Starting worker ...")
        worker = SubprocessWorker(command, queue, lock, env=self.env)
        worker.start()

        # Process the output from the worker process
        self._handle_output(queue, worker)

    def _handle_output(self, queue: Queue, worker: Process):
        ...
        worker.join()  # Ensure the worker has finished before continuing
